classdef Partition<handle
	...
	methods
        ...
        function gridPart(part, G)
            % input: part, Partition
            %        G, part.dim x 1 col integer vector
            % output: grided partition
            % assume: part.domain is not yet partitioned, i.e., celllist =
            % {domain}
            side_length = [];
            for i = 1:1:length(G)
                gi = G(i);
                 side_length(end+1) = ...
                     (part.domain.xmax(i) - part.domain.xmin(i))/gi;
            end
            my_cell_list = ...
                Rec([part.domain.xmin; part.domain.xmin + side_length]);
            for i = 1:1:length(G)
                % expand all cells in my_cell_list along dim = i;
                new_cells = [part.domain];
                direction = zeros(1,part.dim);
                direction(i) = side_length(i);
                for j = 1:1:length(my_cell_list)
                    base_cell = my_cell_list(j);
                    for k = 2:1:G(i)
                        new_cells(end+1) = Rec([base_cell.xmin + (k-1)*direction;...
                            part.domain.xmax + (k-1)*direction]);
                    end
                end
                new_cells(1) = [];
                my_cell_list = [my_cell_list, new_cells];
            end
            part.cell_list = my_cell_list;
        end
