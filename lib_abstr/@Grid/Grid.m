classdef Grid<handle
	...
	methods
		function g = GridPartition(grid)
            ...
            g.grid = grid;
            % sanity check
            if any(cellfun(@length,grid) <= 0)
                error('Invalid grid: unmber of grid points along each dimension must be >= 1')
            end
            
            g.dim = length(grid);
            
            g.domain.xmin = cellfun(@min,grid); 
            g.domain.xmax = cellfun(@max,grid); 
        end
        
        function l = length(obj)
        	% Overload length, i.e., number of cells          
            % length/resolution of grid along each dimension
            L = cellfun('length',obj.grid); 
            l = prod(L);
        end
        
        function max_cord = get_max_cord(g, dim)
            L = cellfun('length',g.grid); 
            max_cord = L(dim);
        end
        ...
        function plotCell_by_idx(gpart, cell_idxs, dx, dy, clr)
            ...
            for i = 1:1:length(cell_Recs)
                ...
                plot([xmin,xmax,xmax,xmin,xmin],...
                     [ymin,ymin,ymax,ymax,ymin],'Linewidth',2,'Color', clr); 
            end
        end
        ...
        function plotArrow(gpart, cell1_idx, cell2_idx, end_txt, clr)
            ...
            plot([x1(1),x2(1)], [x1(2),x2(2)], 'Linewidth', 1,'Color',clr);
            hold on; 
