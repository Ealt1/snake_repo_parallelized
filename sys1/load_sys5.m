...
parfor i = 1:1:H-1
    ...
    mytemp = zeros(1000,100);
    mytemp_p = zeros(1000,100);
    h = 0;
    for p0_plus = P0_plus
        for p1_plus = P1_plus
            for p2_plus = P2_plus
...
ind = find(my_i_k ~= 0);
[i, k, p_coord] = ind2sub(size(my_i_k),ind);
myTensor = tesnor(my_i_k);
p = myTensor([i, k, p_coord])
tau([i,p,k]) = 1;
